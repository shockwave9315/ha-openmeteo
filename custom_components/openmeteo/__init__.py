...
class OpenMeteoDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Networking options from OptionsFlow
            ...
            for _try in range(attempts):
                try:
                    async with async_timeout.timeout(total_timeout):
                        ...
                        async with session.get(URL, params=params, timeout=aiohttp.ClientTimeout(total=total_timeout, connect=connect_timeout)) as response:
